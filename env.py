...
class Topology(object):
    ...
    def load_topology(self):
        ...
        print('nodes: %d, links: %d %d %d \n'%(self.num_nodes, self.num_links,len(self.DG.nodes()),len(self.DG.edges())))
            ...
            for s in range(self.num_nodes):
                for d in range(self.num_nodes):
                    if s != d:
                        ...
                        f.writelines(line+'\n')
#         print('pairs and nodes*nodes-1',self.num_pairs , self.num_nodes*(self.num_nodes-1))
...
#             print("these should be equal total_volume_cnt %s , self.num_nodes*self.num_nodes  %s volumes[0] %s"%(total_volume_cnt,self.num_nodes*self.num_nodes,volumes[0]))
            ...
            for v in range(total_volume_cnt):
                ...
                j = v%self.num_nodes
                if i != j:
#                     print('i %s, j %s v %s traffic %s'%(i,j,v,float(volumes[v])))
